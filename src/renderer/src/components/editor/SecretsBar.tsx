...
export function SecretsBar({ def, project }: { def: TerminalDef; project: Project }) {
  ...
  return (
    <div className="flex shrink-0 flex-wrap items-center gap-2 border-b border-border bg-panel-2/60 px-3 py-1.5 text-xs">
      ...
      <span className="text-muted">Bağlantı şifresi{hasPassword ? ' · kasada' : ''}:</span>
      <input
        className="input w-44 py-0.5 font-mono text-xs"
        type="password"
        value={password}
        placeholder={hasPassword ? 'değiştirmek için yaz' : 'şifre'}
        autoComplete="off"
      ...
      <span className="text-muted">Ek şifreler:</span>
      ...
      {adding ? (
        ...
        <button
          ...
          title="Ek şifre ekle (su, ikinci cihaz…)"
